# Load Testing Script for Pharmacy & Inventory Service
# Sends 100 requests to /api/pharmacies and measures throughput
...
Write-Host "Pharmacy & Inventory Load Testing Script" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
for ($i = 1; $i -le $RequestCount; $i += $ConcurrentRequests) {
    ...
    for ($j = $i; $j -le $batchEnd; $j++) {
        $jobs += Start-Job -ScriptBlock {
            ...
            try {
                $startTime = Get-Date
                $response = Invoke-WebRequest -Uri "$url/api/pharmacies" -Method GET -UseBasicParsing -TimeoutSec 30
                $endTime = Get-Date
