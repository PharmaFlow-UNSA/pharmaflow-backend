...
public class TherapyReminderService {
    ...
    public TherapyReminderResponseDto createReminder(TherapyReminderRequestDto requestDto) {
        ...
        reminder.setDosageInstruction(TextSanitizer.sanitizeOptionalText(requestDto.getDosageInstruction()));
    ...
    private TherapyReminder findReminderById(Long id) {
        return therapyReminderRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Therapy reminder not found with id: " + id));
    }

    private LocalDateTime calculateNextReminderAt(TherapyReminder reminder) {
        LocalDateTime now = LocalDateTime.now(clock);
        LocalDate candidateDate = reminder.getStartDate().isAfter(now.toLocalDate())
    ...
    private List<LocalTime> buildDailySchedule(int frequencyPerDay) {
        ...
        return java.util.stream.IntStream.range(0, frequencyPerDay)
                .mapToObj(index -> {
                    long offsetMinutes = Math.round((double) availableWindowMinutes * index / (frequencyPerDay - 1));
