...
public class StartupSummaryLogger {
    ...
    public void logStartupSummary() {
        ...
        logDatabaseStatus();
        log.info("Modules: pharmacies, inventory, reservations, deliveries, inventory reservation saga");
        log.info("Demo inventory: seeded on startup when repositories are empty and profile is not test");
        log.info(
        ...
        log.info(
                "RabbitMQ saga: enabled={}, exchangeConfigured={}, inventoryQueuesConfigured={}, orderQueuesConfigured={}",
                enabled("pharmaflow.rabbitmq.enabled", true),
                hasText(environment.getProperty("pharmaflow.rabbitmq.exchange")),
                hasText(environment.getProperty("pharmaflow.rabbitmq.inventory-reservation-requested-queue")),
                hasText(environment.getProperty("pharmaflow.rabbitmq.order-placed-queue")));
    }
    ...
    private String appName() {
        return environment.getProperty("spring.application.name", "pharmacy-inventory-service");
    }

    private String port() {
        return environment.getProperty("server.port", "8084");
    }
