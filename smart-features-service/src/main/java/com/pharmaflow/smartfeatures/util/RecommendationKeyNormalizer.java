...
public final class RecommendationKeyNormalizer {
    ...
    public static String buildActiveDimensionKey(
            Long userId, Long patientProfileId, Long productId, RecommendationType recommendationType, RecommendationStatus status) {
        if (status != RecommendationStatus.ACTIVE) {
            return null;
        }

        return userId + "|" + normalizeNullable(patientProfileId) + "|" + productId + "|" + recommendationType.name();
    }
