...
public class NotificationService {
  ...
  @Transactional
  public NotificationResponseDto updateDeliveryStatus(
      Long id, NotificationDeliveryStatusRequestDto requestDto) {
    ...
    if (targetStatus == NotificationStatus.READ || targetStatus == NotificationStatus.PENDING) {
      throw new BadRequestException(
          "Use mark-read for READ status and create endpoint for PENDING status.");
    }
  ...
  private TherapyReminder findReminderOrNull(Long reminderId) {
    ...
    return therapyReminderRepository
        ...
        .orElseThrow(
            () ->
                new ResourceNotFoundException("Therapy reminder not found with id: " + reminderId));
  }

  private void createTrigger(Notification notification, NotificationType notificationType) {
    NotificationTriggerSource source =
        switch (notificationType) {
          ...
          case SYSTEM -> NotificationTriggerSource.SYSTEM;
        };
  ...
  private void validateTransition(
      NotificationStatus currentStatus, NotificationStatus targetStatus) {
    boolean valid =
        switch (currentStatus) {
          case PENDING ->
              targetStatus == NotificationStatus.SENT || targetStatus == NotificationStatus.FAILED;
          case SENT ->
    ...
    if (!valid) {
      throw new BadRequestException(
          "Invalid notification status transition from "
              + currentStatus
              + " to "
              + targetStatus
              + ".");
